...
final class SQLiteDatabase {
    ...
    // SQLite handles are not safe to hit from multiple threads at once unless you
    // serialize access yourself (MushafViewModel fires concurrent loads for the
    // current/next/previous page). All queries go through this serial queue.
    private let queue = DispatchQueue(label: "com.mushaf.sqlitedatabase")

    init(path: String) throws {
        var handle: OpaquePointer?
        // FULLMUTEX makes the connection itself safe to call into from any thread;
        // the serial queue above additionally prevents interleaved statement use.
        ...
        try queue.sync {
            ...
            for (index, value) in bind.enumerated() {
                ...
                case let v as String:
                    ...
                    // immediately, instead of trusting a pointer into a bridged NSString
                    // whose lifetime we don't control.
